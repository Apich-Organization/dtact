...
impl Worker {
    ...
    /// paths enforce this via `LOCAL_QUEUE_HIGH_WATERMARK`. `tail` must be
    /// pre-calculated to avoid redundant atomic loads.
    #[inline]
...
impl DtaScheduler {
    ...
    fn route_chunk(
        ...
        if space_ok {
            ...
            self.route_local(worker, current_core, chunk, cur_len, fixed_head);
            added
        } else if hops_ok {
