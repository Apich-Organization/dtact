...
impl Worker {
    ...
    pub fn push_local(&self, task: TaskIndex) -> bool {
        // Bolt Optimization: Inline queue length calculation to avoid redundant load of `local_tail`.
        let tail = self.local_tail.load(Ordering::Relaxed);
        let head = self.local_head.load(Ordering::Relaxed);
        let cur_len = tail.wrapping_sub(head) & LOCAL_QUEUE_MASK;
        if cur_len >= LOCAL_QUEUE_CAPACITY - 1 {
            return false;
        }
        unsafe {
...
impl DtaScheduler {
    ...
    pub fn drain_warehouse(&self, current_core: usize) -> bool {
        ...
        // Bolt Optimization: Cache `local_head` outside the loop to avoid redundant atomic loads.
        ...
        while drained < cap {
            ...
            let cur_len = tail.wrapping_sub(fixed_head) & LOCAL_QUEUE_MASK;
    ...
    fn route_chunk(
        ...
        // Bolt Optimization: Use the passed `fixed_head` to calculate length, saving an atomic load.
        let tail = worker.local_tail.load(Ordering::Relaxed);
