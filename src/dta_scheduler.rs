...
impl Worker {
    ...
    ///
    /// Note: `cur_tail` is passed as a mutable reference to cache the atomic load
    /// of `local_tail` across batch pushes, reducing redundant memory accesses in hot loops.
    #[inline]
    pub fn push_batch(&mut self, chunk: &TaskChunk, cur_tail: &mut usize) {
...
impl DtaScheduler {
    ...
    pub fn drain_warehouse(&self, current_core: usize) -> bool {
        ...
        let mut cur_tail = worker.local_tail.load(Ordering::Relaxed);
        let mut cur_len = cur_tail.wrapping_sub(fixed_head) & LOCAL_QUEUE_MASK;
        ...
        while drained < cap {
            ...
            match self.warehouse.pop() {
                Some(chunk) => {
                    worker.push_batch(&chunk, &mut cur_tail);
    ...
    fn route_chunk(
        ...
        if space_ok {
            ...
            self.route_local(worker, current_core, chunk, cur_tail);
            added
        } else if hops_ok {
