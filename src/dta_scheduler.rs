...
impl DtaScheduler {
    ...
    pub fn drain_warehouse(&self, current_core: usize) -> bool {
        ...
        while drained < cap {
            ...
            if local_len + CHUNK_SIZE > LOCAL_QUEUE_HIGH_WATERMARK {
                break;
            }
    ...
    ///
    /// Accepts a pre-loaded `fixed_head` since the caller (`poll_mailboxes`)
    /// does not modify `local_head` during execution, saving redundant atomic loads.
    #[inline(always)]
