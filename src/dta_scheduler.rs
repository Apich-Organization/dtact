...
impl DtaScheduler {
    ...
    pub fn drain_warehouse(&self, current_core: usize) -> bool {
        ...
        // local_head is not modified by this loop (only pushing work)
    ...
    fn route_chunk(
        ...
        let local_tail = worker.local_tail.load(Ordering::Relaxed);
        let local_len = local_tail.wrapping_sub(fixed_head) & LOCAL_QUEUE_MASK;
