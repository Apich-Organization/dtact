...
impl DtaScheduler {
    ...
    pub fn drain_warehouse(&self, current_core: usize) -> bool {
        ...
        // local_tail is mutated only by this thread. Hoist the load and update it locally
        // to avoid redundant atomic reads in the loop.
        let mut fixed_tail = worker.local_tail.load(Ordering::Relaxed);

        while drained < cap {
            let cur_len = fixed_tail.wrapping_sub(fixed_head) & LOCAL_QUEUE_MASK;
            ...
            match self.warehouse.pop() {
                Some(chunk) => {
                    ...
                    fixed_tail = fixed_tail.wrapping_add(chunk.count as usize);
                    drained += 1;
                }
                None => break,
            }
    ...
    /// not change here (we are only pushing work, not consuming).
    /// `local_tail` is also cached and updated locally to eliminate
    /// all redundant atomic reads during capacity checks in the hot loop.
    #[inline(always)]
        ...
        for idx in 0..num_polls {
            ...
            loop {
                ...
                match row[current_core].pop() {
                    Some(chunk) => {
                        received_any = true;
                        let pushed = self.route_chunk(worker, current_core, chunk, cur_len);
                        fixed_tail = fixed_tail.wrapping_add(pushed);
                    }
                    None => break,
                }
            }
    ...
    fn route_chunk(
        ...
        if space_ok {
            let pushed_count = chunk.count as usize;
